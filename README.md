# Aura — AI-Powered Process Optimizer

**Aura** is a Linux process management tool that combines real-time system monitoring with Anthropic Claude AI to intelligently identify and terminate wasteful processes. It features an htop-like terminal UI, automatic and interactive modes, power savings tracking, and a multi-layered safety system to protect critical services.
...
```bash
./aura interactive
./aura interactive --config /path/to/config.yaml
```
...
Starts Aura in background daemon mode.

```bash
./aura start --daemon          # Background daemon
./aura start                   # Foreground (same as monitor)
```

PID file stored at `/tmp/aura.pid`.
...
View or follow the Aura log file.

```bash
./aura logs                    # Print entire log
./aura logs --follow           # Tail -f style
```
...
Aura searches for configuration in this order:

1. Path specified via `--config` flag
2. `./config.yaml` (current directory)
3. `~/.aura/config.yaml` (user home)
4. `/etc/aura/config.yaml` (system-wide)

Environment variables override config file values with the `AURA_` prefix (e.g., `AURA_MONITORING_SCAN_INTERVAL`). The `ANTHROPIC_API_KEY` environment variable is read directly.
...
notifications:
  log_file: "aura.log"           # Application log file
  audit_file: "aura-audit.log"   # AI decision audit trail
...
Aura implements multiple layers of protection to prevent terminating critical processes.
...
Aura categorizes every process into one of four categories:
...
### Known System Daemons (automatically classified)
...
Aura reads directly from the Linux `/proc` filesystem:
...
- Forks a background `aura monitor` process
- Writes PID to `/tmp/aura.pid`
...
Aura maintains an append-only JSON audit log at the configured `audit_file` path (default: `aura-audit.log`). Every AI decision and termination is recorded:
...
│   ├── monitor.go                    # aura monitor — text process viewer
│   ├── yolo.go                       # aura yolo — automatic AI mode
│   ├── interactive.go                # aura interactive — TUI launcher
│   ├── status.go                     # aura status — system status
│   ├── start.go                      # aura start --daemon
│   ├── stop.go                       # aura stop
│   └── logs.go                       # aura logs --follow
├── internal/
│   ├── config/
