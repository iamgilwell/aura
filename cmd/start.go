...
var startCmd = &cobra.Command{
	Use:   "start",
	Short: "Start Aura in background daemon mode",
	RunE:  runStart,
}
...
func runStart(cmd *cobra.Command, args []string) error {
	pidFile := filepath.Join(os.TempDir(), "aura.pid")
		...
		if pid, err := strconv.Atoi(pidStr); err == nil {
			if process, err := os.FindProcess(pid); err == nil {
				...
				if err := process.Signal(os.Signal(nil)); err == nil {
					return fmt.Errorf("Aura daemon already running (PID: %d)", pid)
				}
			}
		}
	}
	...
	if !daemon {
		fmt.Println("Starting Aura in foreground monitoring mode...")
	...
	fmt.Println("Use 'aura stop' to stop the daemon.")
	fmt.Println("Use 'aura logs --follow' to watch the log output.")
